...
int main(int argc, char *argv[])
{
   ...
   for (int iter = 0; iter < 1000; iter++){
      #pragma omp parallel
      {
         struct timespec tstart_stencil;
         if (thread_id == 0) cpu_timer_start(&tstart_stencil);
         ...
         if (thread_id == 0) SWAP_PTR(xnew, x, xtmp);

         if (thread_id == 0) stencil_time += cpu_timer_stop(tstart_stencil);
